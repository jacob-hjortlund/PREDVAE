...
class PhotometryDataset(eqx.Module):
    ...
    def __init__(
        ...
        self.psf_color_mean = jnp.mean(psf_colors, axis=0)
        self.psf_color_std = jnp.std(psf_colors, axis=0)
        ...
        self.model_color_mean = jnp.mean(model_colors, axis=0)
        self.model_color_std = jnp.std(model_colors, axis=0)
        ...
        self.additional_features_mean = (
            jnp.mean(additional_features, axis=0)
            if additional_features is not None
            else np.zeros((0,))
        )
        self.additional_features_std = (
            jnp.std(additional_features, axis=0)
            if additional_features is not None
            else np.ones((0,))
        )
        ...
        self.log10_redshift_mean = (
            jnp.mean(log10_redshift, axis=0)
            if log10_redshift is not None
            else jnp.zeros((len(psf_photometry), 1))
        )
        self.log10_redshift_std = (
            jnp.std(log10_redshift, axis=0)
            if log10_redshift is not None
            else jnp.ones((len(psf_photometry), 1))
        )
